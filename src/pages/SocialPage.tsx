    ...
    return (
        <div className="max-w-2xl mx-auto">
            ...
            <div className="bg-zinc-900/50 p-4 rounded-xl border border-zinc-800 mb-8 flex gap-4">
                ...
                />
                <button className="text-ritual-accent font-bold text-sm uppercase">Post</button>
            </div>

            <div className="space-y-6">
                {MOCK_POSTS.map(post => (
                    <div key={post.id} className="border-l-2 border-zinc-800 pl-6 py-2 hover:border-ritual-accent transition-colors">
                        <div className="flex items-center gap-2 mb-2">
                            <span className={`font-bold ${post.isSystem ? 'text-ritual-accent' : 'text-white'}`}>
                                @{post.author}
                            </span>
