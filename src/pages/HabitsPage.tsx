...
const HabitsPage = ({ 
  ...
  setUser: Function
    ...
    const toggleCompletion = (ritual: Ritual) => {
        const isCompleted = logs.some(l => l.ritualId === ritual.id && l.date === todayStr);
        
        if (isCompleted) {
            // --- UNDO LOGIC ---
            const newLogs = logs.filter(l => !(l.ritualId === ritual.id && l.date === todayStr));
            setLogs(newLogs);

            const yesterday = new Date();
            yesterday.setDate(yesterday.getDate() - 1);
            const yesterdayStr = yesterday.toISOString().split('T')[0];
            const hadStreakYesterday = newLogs.some(l => l.ritualId === ritual.id && l.date === yesterdayStr);
            ...
            setUser({ ...user, essence: Math.max(0, user.essence - ritual.essenceReward - bonusToRemove) });
            setRituals(rituals.map(r => r.id === ritual.id ? { ...r, streak: recoveredStreak } : r));
            ...
            yesterday.setDate(yesterday.getDate() - 1);
            const yesterdayStr = yesterday.toISOString().split('T')[0];
            const hasYesterday = logs.some(l => l.ritualId === ritual.id && l.date === yesterdayStr);
            ...
            setUser({ ...user, essence: user.essence + ritual.essenceReward + streakBonus });
            setRituals(rituals.map(r => r.id === ritual.id ? { ...r, streak: newStreak } : r));
        }
    };
        ...
        if (createMode === 'ai') {
            ...
            if (suggestions.length > 0) {
                const newRituals: Ritual[] = suggestions.map((s: any, idx: number) => ({
                    ...
                    frequency: 'daily',
                    essenceReward: s.difficulty === 'master' ? 50 : s.difficulty === 'adept' ? 25 : 10,
                    streak: 0
                }));
        ...
        <div className="max-w-3xl mx-auto space-y-8">
                ...
                <div className="bg-zinc-900 p-6 rounded-xl border border-zinc-800 shadow-xl animate-fade-in-down">
                    <div className="flex gap-4 mb-6 border-b border-zinc-800 pb-4">
                        <button 
                            ...
                            className={`flex items-center gap-2 text-sm font-bold uppercase tracking-wider pb-1 ${createMode === 'manual' ? 'text-ritual-accent border-b-2 border-ritual-accent' : 'text-zinc-500 hover:text-zinc-300'}`}
                        >
                            <PenTool size={14} /> Manual Entry
                        </button>
                        <button 
                            onClick={() => setCreateMode('ai')} 
                            className={`flex items-center gap-2 text-sm font-bold uppercase tracking-wider pb-1 ${createMode === 'ai' ? 'text-ritual-accent border-b-2 border-ritual-accent' : 'text-zinc-500 hover:text-zinc-300'}`}
                        >
                    ...
                    <form onSubmit={handleCreateRitual} className="space-y-4">
                        {createMode === 'ai' ? (
                            <>
                                <p className="text-zinc-400 text-sm mb-2">State your desire (e.g., "Become physically unstoppable" or "Learn to code"). The system will prescribe your suffering.</p>
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className="text-xs text-zinc-500 uppercase font-bold block mb-1">Ritual Title</label>
                                    <input 
                                        type="text" 
                                ...
                                <div>
                                    <label className="text-xs text-zinc-500 uppercase font-bold block mb-1">Description</label>
                                    <input 
                                        type="text" 
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div>
                                        <label className="text-xs text-zinc-500 uppercase font-bold block mb-1">Difficulty</label>
                                        <select 
                                    ...
                                    <div>
                                        <label className="text-xs text-zinc-500 uppercase font-bold block mb-1">Frequency</label>
                                        <select 
                                            value={manualForm.frequency}
                                            onChange={(e) => setManualForm({...manualForm, frequency: e.target.value as any})}
                                            className="w-full bg-black border border-zinc-700 rounded-lg px-4 py-2 text-white focus:border-ritual-accent outline-none appearance-none"
                                        >
                                    ...
                                    <div>
                                        <label className="text-xs text-zinc-500 uppercase font-bold block mb-1">Essence Reward</label>
                                        <input 
                                            type="number" 
                                            value={manualForm.essenceReward}
                                            onChange={(e) => setManualForm({...manualForm, essenceReward: Number(e.target.value)})}
                                            className="w-full bg-black border border-zinc-700 rounded-lg px-4 py-2 text-white focus:border-ritual-accent outline-none"
                                        />
                                    </div>
                                </div>
            ...
            <div className="space-y-3">
                {rituals.length === 0 ? (
                    ...
                    rituals.map(ritual => {
                        const isDone = logs.some(l => l.ritualId === ritual.id && l.date === todayStr);
                        return (
                           <RitualItem 
                                key={ritual.id} 
                                ritual={ritual} 
