...
const LandingPage = ({ onAuth }: { onAuth: (user: User) => void }) => {
  ...
  React.useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.from('.feature-card', { y: 30, opacity: 0, duration: 0.8, stagger: 0.1, delay: 0.5 });
    });
    return () => ctx.revert();
  }, []);
    ...
    <div className="min-h-screen bg-black text-white selection:bg-ritual-accent selection:text-black font-sans overflow-hidden relative">
        ...
        <nav className="p-6 flex justify-between items-center relative z-10 max-w-7xl mx-auto w-full">
            <div className="flex items-center gap-2">
                ...
                <span className="font-display font-bold text-xl tracking-widest">RitualKiller</span>
            </div>
            <button onClick={handleLogin} className="text-sm font-semibold hover:text-ritual-accent transition-colors">INITIATE_LOGIN</button>
        </nav>
        ...
        <header className="flex flex-col items-center justify-center min-h-[80vh] text-center px-4 relative z-10">
            <h1 className="hero-text text-5xl md:text-8xl font-display font-black mb-6 uppercase tracking-tighter">
                Kill The <span className="text-transparent bg-clip-text bg-gradient-to-r from-ritual-accent to-emerald-800">Old You</span>
            </h1>
            ...
            >
                <span className="relative z-10 flex items-center gap-2">Begin The Ritual <Play size={16} fill="black" /></span>
        ...
        <section className="max-w-7xl mx-auto px-4 py-20 grid md:grid-cols-3 gap-8">
            {[
                { title: 'Rituals', desc: 'Define your daily sacrifices. Strict tracking creates iron discipline.' },
                { title: 'Heatmap', desc: 'Visualize your consistency. Watch your dormant potential awaken.' },
                { title: 'Black Market', desc: 'Exchange your suffering (effort) for digital glory.' }
            ].map((f, i) => (
                <div key={i} className="feature-card p-8 border border-zinc-900 bg-zinc-950/50 hover:border-ritual-accent/50 transition-colors">
                    <h3 className="font-display text-2xl font-bold mb-4 text-zinc-200">{f.title}</h3>
